...
const FALLBACK: BrandingInitial = { name: 'Centro Cultural', logoUrl: null, credentialLogoUrl: null, logoScale: 100, primaryColor: '#e65100', secondaryColor: '#ff6f00', sidebarTheme: 'brand' };

export default async function IdentidadPage() {
  const branding = await getTenantBranding();
  const gate = await getAdminGate();
  const initial: BrandingInitial = gate.status === 'ok'
    ? { name: gate.branding.name, logoUrl: gate.branding.logoUrl, credentialLogoUrl: gate.branding.credentialLogoUrl, logoScale: gate.branding.logoScale, primaryColor: gate.branding.primaryColor, secondaryColor: gate.branding.secondaryColor, sidebarTheme: gate.branding.sidebarTheme }
    : FALLBACK;
