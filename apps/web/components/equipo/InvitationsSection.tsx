...
const ROLE_LABEL: Record<StaffInvitation['role'], string> = { owner: 'Propietario', admin: 'Administrador', operator: 'Operador', protocolo: 'Protocolo', consulta: 'Consulta', puerta: 'Puerta', biblioteca: 'Biblioteca' };
...
export function InvitationsSection({ canInviteOwner }: { canInviteOwner: boolean }) {
  ...
  const [role, setRole] = useState<'admin' | 'operator' | 'owner' | 'protocolo' | 'consulta' | 'puerta' | 'biblioteca'>('operator');
